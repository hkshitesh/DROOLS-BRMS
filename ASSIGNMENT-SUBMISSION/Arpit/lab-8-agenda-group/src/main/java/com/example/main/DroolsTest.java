...
public class DroolsTest {
    public static void main(String[] args) {
        ...
        var product1a = Product.builder().name("Product 1a").category("Electronics").price(100).build();
        ...
        kSession.getAgenda().getAgendaGroup("round-0").setFocus();
        kSession.getAgenda().getAgendaGroup("round-1").setFocus();
        kSession.fireAllRules();

        System.out.println("\n\n");

        kSession.insert(product1a);
        kSession.insert(Order.builder().name("Order 1a").product(product1a).quantity(6).build());
